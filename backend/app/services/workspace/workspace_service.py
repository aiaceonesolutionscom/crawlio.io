...
async def workspace_usage_counts(
    session: AsyncSession, workspace_ids: list[str]
) -> dict[str, dict[str, int]]:
    """Usage numbers for the admin panel: seats filled, leads stored and emails
    sent for each workspace. One round-trip per table (batched IN query)."""
    counts: dict[str, dict[str, int]] = {}
    if not workspace_ids:
        return counts
    for workspace_id in workspace_ids:
        counts[workspace_id] = {"member_count": 0, "lead_count": 0, "email_count": 0}
    ...
    for workspace_id, count in members_result.all():
        counts[workspace_id]["member_count"] = count

    leads_result = await session.execute(
        select(Lead.workspace_id, func.count())
        .where(Lead.workspace_id.in_(workspace_ids))
        .group_by(Lead.workspace_id)
    )
    for workspace_id, count in leads_result.all():
        counts[workspace_id]["lead_count"] = count

    emails_result = await session.execute(
        select(EmailMessage.workspace_id, func.count())
        .where(EmailMessage.workspace_id.in_(workspace_ids))
        .group_by(EmailMessage.workspace_id)
    )
    for workspace_id, count in emails_result.all():
        counts[workspace_id]["email_count"] = count
