...
const PLAN_BADGE: Record<PlanId, string> = {
  free: 'rounded bg-ink-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-chalk-dim',
  pro: 'rounded bg-signal px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-signal-deep',
  enterprise: 'rounded bg-signal px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-signal-deep',
};

export function DashboardLayout({ tier }: Props) {
  const { user, logout } = useSession();
  const navigate = useNavigate();
  const [navOpen, setNavOpen] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);
  const [upgradeOpen, setUpgradeOpen] = useState(false);
  const NAV = buildNav(tier);
  ...
  <nav aria-label="Dashboard" className="flex h-full flex-col">
      <div className="flex h-16 items-center px-5">
        <Link to={`/app/${tier}`} aria-label="Crawlio dashboard">
          <Logo />
        </Link>
      </div>
      ...
      <div className="border-t border-ink-850 p-4">
        ...
        <p className="mt-1.5 font-display text-[16px] font-semibold text-chalk">{PLAN_LABEL[tier]}</p>
        {tier !== 'enterprise' &&
        ...
        <header className="sticky top-0 z-40 flex h-16 items-center gap-3 border-b border-ink-850 bg-ink-950/90 px-4 backdrop-blur-xl sm:px-6">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <span className={PLAN_BADGE[tier]}>
                {PLAN_LABEL[tier]}
              </span>
            </div>
          </div>
